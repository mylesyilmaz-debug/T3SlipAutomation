...
public class GCPStoragePage extends PageObject {
    private static final Logger logger = LogManager.getLogger(GCPStoragePage.class);
    // --- 1. Locators ---
    ...
    // --- Rejects Reconciliation Logic ---

    // --- SPECIFIC Rejects Reconciliation Logic ---

    public boolean isSpecificRejectFileVisible(String fileName) {
        logger.info("Checking if Reject file '" + fileName + "' exists in the GCP UI...");
        try {
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(3));
            // Look specifically for the exact filename provided
            boolean isPresent = !driver.findElements(By.partialLinkText(fileName)).isEmpty();
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(15)); // Reset to default
            return isPresent;
        } catch (Exception e) {
            return false;
        }
    ...
    public String getSpecificRejectFileSizeInUI(String fileName) {
        try {
            // Find the table row that contains this exact file name
            WebElement fileRow = driver.findElement(By.xpath("//tr[.//a[contains(text(), '" + fileName + "')]]"));
            return fileRow.getText(); // We will parse this to see if it says "0 B"
        } catch (Exception e) {
            return "UNKNOWN";
        }
    }

    public void downloadRejectFile() {
        logger.info("Attempting to download the Reject file...");
        WebDriverWait localWait = new WebDriverWait(driver, Duration.ofSeconds(15));

        try {
            WebElement rejectFile = localWait.until(ExpectedConditions.elementToBeClickable(
                    By.xpath("//a[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'reject')]")
            ));
            rejectFile.click();

            WebElement downloadBtn = localWait.until(ExpectedConditions.elementToBeClickable(
            ...
            throw new RuntimeException("Failed to download Reject file: " + e.getMessage());
        }
    }
}